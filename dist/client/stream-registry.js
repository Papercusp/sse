...
exports.STREAM_BUDGET_WARN_AT = void 0;
exports.listLiveStreams = listLiveStreams;
exports.countLiveStreamsForHost = countLiveStreamsForHost;
exports.registerLiveStream = registerLiveStream;
exports._resetStreamRegistry = _resetStreamRegistry;
/**
 ...
 */
exports.STREAM_BUDGET_WARN_AT = 4;
const live = new Map();
let nextId = 1;
...
function countLiveStreamsForHost(host) {
    let n = 0;
 ...
 */
function registerLiveStream(url) {
    const id = nextId++;
    const host = hostOf(url);
    live.set(id, { id, url, host, openedAtMs: Date.now() });
    const n = countLiveStreamsForHost(host);
    if (n >= exports.STREAM_BUDGET_WARN_AT && !warnedHosts.has(host)) {
        ...
        console.warn(`[sse-budget] ${n} standing SSE streams open to ${host}. Browser engines allow ~6 ` +
    ...
    return () => {
        ...
        if (countLiveStreamsForHost(host) < exports.STREAM_BUDGET_WARN_AT)
            warnedHosts.delete(host);
    };
}
/** Test seam — drop all registry state. */
function _resetStreamRegistry() {
    live.clear();
    warnedHosts.clear();
    nextId = 1;
}
